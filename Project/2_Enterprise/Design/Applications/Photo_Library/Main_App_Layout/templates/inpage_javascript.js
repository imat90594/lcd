$(document).ready(function() {
	...
	$(document).on("click", "#delete_image input[name=delete_photo]", function(e){
		e.preventDefault();
		
		$.php("/ajax/photo_library/images/deleteImage",$("#delete_image").serialize());
		
		php.complete = function(){
			var data = $("#delete_image input[name=image_id]").val();
...
//			$("#photo_details_holder").fadeIn();
	...
	//-----------------------------------------------------------------------------------------------------------
	//SORT ALBUMS OR IMAGES
	$('.imagesContainer, .albumContainer').sortable().bind('sortupdate', function(e, ui) {
		var items    = new Array();
		var is_album = ($(this).hasClass("albumContainer")) ? true : false; //True if sorting album, false if sorting image
		var action   = (is_album) ? 'sortAlbums' : 'sortImages'; 
		
		if(is_album){
			$("div.sortableAlbums").each(function(){
				var album_id = $(this).attr('id').replace("album_", "");
				
				items.push({album_id:album_id});
			});
		}
		else
		{
			$("div.sortableImages").each(function(){
				var image_id = $(this).attr('id').replace("image", "");
				
				items.push({image_id:image_id});
			});
		}
		
		$('.modal-backdrop').addClass('in').removeClass('hide');
		
		$.php("/ajax/photo_library/albums/"+action, {items:items});
	...
	$("input[name=is_used]").change(function(){
		...
		$.php("/ajax/photo_library/images/isUsed", {album_id:album_id, val:val});
	});
