		...
		try
		{
			...
			if($this->category != NULL)
			{
				if(isset($this->category["permalink"]))				
					...
					$sql .= " AND ac.article_category_id = :article_category_id";
			}
			...
			if($this->category != NULL)
			{
				if(isset($this->category["permalink"]))
					...
					$pdo_statement->bindParam(":article_category_id", $this->category["article_category_id"], PDO::PARAM_STR);
			}
	...
	public static function selectByColumn($column, $value)
	{
		try
		{
			...
			$sql = "SELECT
						...
						r.route_id,
						r.page_id,
						r.permalink as article_permalink,
						ac.*,
						i.image_title,
						i.image_caption,
						CONCAT('/Data/Images/', ab.album_folder, '/', ais.dimensions, '/', i.path) as image_path,
